...
function buildClientReadyRiskReport(params: {
  ...
  const { result, sourceText, fileName, template } = params;
  const generatedAt = new Date().toLocaleString();

  const lines: string[] = [];

  lines.push("HUGINN AI — CONTRACT ANALYSIS REPORT");
  lines.push("Generated by Oden's Eye Creative");
  lines.push("=".repeat(58));
  lines.push("");
  lines.push(`Generated: ${generatedAt}`);
  lines.push(`Template: ${formatTemplateLabel(template)}`);
  lines.push(`Source File: ${fileName || "Pasted text"}`);
  lines.push(`Document Type: ${toTitleCase(result.documentType)}`);
  lines.push(`Risk Score: ${result.riskScore ?? 0} / 100`);
  lines.push(`Risk Level: ${titleCaseRiskLevel(result.riskLevel)}`);
  lines.push("");
  lines.push("SUMMARY");
  lines.push("-".repeat(58));
  ...
  if ((result.issues?.length ?? 0) === 0) {
    ...
    (result.issues ?? []).forEach((issue, index) => {
      ...
      lines.push(`Severity: ${titleCaseRiskLevel(issue.severity)}`);
      lines.push("");
  ...
  if ((result.deadlines?.length ?? 0) === 0) {
    lines.push("No deadlines extracted.");
  } else {
    (result.deadlines ?? []).forEach((deadline, index) => {
      lines.push(`${index + 1}. ${deadline.label}: ${deadline.value}`);
    });
  }

  lines.push("");
  lines.push("SOURCE TEXT PREVIEW");
  lines.push("-".repeat(58));
  lines.push(sourceText.slice(0, 2000) || "No source text available.");
  lines.push("");
  lines.push("Generated by Huginn AI");
  lines.push("Oden's Eye Creative");
      ...
      <main className="pageShell">
        <section className="w-full py-28 px-6 text-center bg-gradient-to-b from-white to-gray-100">
          <h1 className="text-4xl font-bold mb-6">
            Huginn AI — Contract Intelligence for Businesses
          </h1>
        ...
        <section className="py-16 px-6 bg-gray-50">
          ...
          <div className="grid md:grid-cols-3 gap-10 max-w-5xl mx-auto text-center">
            ...
            <div>
              ...
              <p className="text-gray-600">
                Huginn AI scans the contract to detect risks, compliance gaps, and obligations.
              </p>
            </div>
            ...
            {isAnalyzing ? (
              ...
              <div className="emptyState">
                ...
                <p className="emptyText">
                  Upload a file to see the risk score, issue breakdown,
                  document classification, deadlines, and recommendations.
                </p>
              </div>
            ) : (
              <>
